...
class PaymentService:
    ...
    async def create_payment(self, payment_data: PaymentCreate, idempotency_key: str = None) -> Payment:
        # Check idempotency
        if idempotency_key:
    ...
    async def initiate_payment(self, transaction_id: str) -> Payment:
        ...
        if payment.status != PaymentStatus.PENDING:
            raise HTTPException(status_code=400, detail=f"Payment cannot be initiated. Current status: {payment.status.value}")
